...
class UpdateForProduct(generics.UpdateAPIView):
    ...
    def get(self,request,*args,**kwargs):
        try:
            queryset,status = ModelController(table=Product).get_one_content(val=kwargs['pk'])
            if status:
...
class UpdateForCategory(generics.UpdateAPIView):
    ...
    def get(self,request,*args,**kwargs):
        try:
            queryset,status = ModelController(table=ChartOfAccounts).get_one_content(val=kwargs['pk'])
            if status:
...
class UpdateForSubCategory(generics.UpdateAPIView):
    ...
    def get(self,request,*args,**kwargs):
        try:
            queryset,status = ModelController(table=SubCategory).get_one_content(val=kwargs['pk'])
            if status:
...
class UpdateForOrderItem(generics.UpdateAPIView):
    ...
    def get(self,request,*args,**kwargs):
        try:
            queryset,status = ModelController(table=OrderItems).get_one_content(val=kwargs['pk'])
            if status:
...
class UpdateForCoupon(generics.UpdateAPIView):
    ...
    def get(self,request,*args,**kwargs):
        try:
            queryset,status = ModelController(table=Coupon).get_one_content(val=kwargs['pk'])
            if status:
    ...
    def patch(self, request, *args, **kwargs):
        ...
        serializer = CouponSerializer(instance, data=request.data, partial=True)
        if serializer.is_valid():
            data = serializer.save()
            return Response({"data":serializer.data,"status":"success"})
        else:
...
class UpdateForWishlist(generics.UpdateAPIView):
    ...
    def get(self,request,*args,**kwargs):
        try:
            queryset,status = ModelController(table=Wishlist).get_one_content(val=kwargs['pk'])
            if status:
...
class UpdateForAttributes(generics.UpdateAPIView):
    ...
    def get(self,request,*args,**kwargs):
        try:
            queryset,status = ModelController(table=Attributes).get_one_content(val=kwargs['pk'])
            if status:
    ...
    def patch(self, request, *args, **kwargs):
        ...
        serializer = AttributesSerializer(instance, data=request.data, partial=True)
        if serializer.is_valid():
            data = serializer.save()
            return Response({"data":serializer.data,"status":"success"})
        else:
...
class UpdateForBanner(generics.UpdateAPIView):
    ...
    def get(self,request,*args,**kwargs):
        try:
            queryset,status = ModelController(table=BannerSettings).get_one_content(val=kwargs['pk'])
            if status:
    ...
    def patch(self, request, *args, **kwargs):
        ...
        serializer = BannerSerializer(instance, data=request.data, partial=True)
        if serializer.is_valid():
            data = serializer.save()
            return Response({"data":serializer.data,"status":"success"})
        else:
...
class UpdateForBillingAddress(generics.UpdateAPIView):
    ...
    def get(self,request,*args,**kwargs):
        try:
            queryset,status = ModelController(table=BillingAddress).get_one_content(val=kwargs['pk'])
            if status:
    ...
    def patch(self, request, *args, **kwargs):
        ...
        serializer = BillingAddressSerializer(instance, data=request.data, partial=True)
        if serializer.is_valid():
            data = serializer.save()
            return Response({"data":serializer.data,"status":"success"})
        else:
...
class UpdateForOrder(generics.UpdateAPIView):
    ...
    def get(self,request,*args,**kwargs):
        try:
            queryset,status = ModelController(table=Order).get_one_content(val=kwargs['pk'])
            if status:
